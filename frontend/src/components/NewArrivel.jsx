...
const NewArrivel = () => {
  ...
  return (
    <div className="border-2 bg-blue-400 mx-10 my-2">
        <h1 className="text-center text-3xl font-serif my-9 text-white">NEW ARRIVELS</h1>
    <div className="w-[1053px] h-[450px] m-auto mt-5  gap-2">
      <Slider {...settings}>
      {data.map((item, index) => {
          return (
            <div className='gap-2 mx-auto block'>
            <div key={index} className="bg-cyan-600 rounded-lg border-2 w-[205px] h-[400px] gap-3">
              <div className=" ">
                <div className="w-[200px] h-[230px] rounded-lg">
                   <img src={item.image} alt="" className="w-full h-full rounded-lg"/>
                </div>
                <h2 className="text-center uppercase text-2xl font-serif text-white">{item.title}</h2>
                <p className='text-xl text-center text-gray-50'>{item.descript}</p>
